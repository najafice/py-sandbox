...
# op.recorder('mpco', 'results.hdf5', '-N', 'displacement', 'velocity', 'acceleration', 'pressure')
...
for i in range(1,nElemT+1):
    op.parameter(int(ctr+1.0), 'element', i, 'vPerm')
    op.parameter(int(ctr+2.0), 'element', i, 'hPerm')
    ctr   = ctr+2.0
...
for j in range(1,nElemT+1):
    ...
    for i in range(1,numLayers+1):
        if j * sElemY[i-1] <= layerBound[i-1] and j*sElemY[i-1] > lowerBound:
            op.updateParameter(int(ctr+1.0), vPerm[i-1])
            op.updateParameter(int(ctr+2.0), hPerm[i-1])
        lowerBound = layerBound[i-1]
    ctr = ctr+2.0
...
# creating mpco recorder
pvd_directory_name = "results"

# Create the directory
try:
    os.mkdir(pvd_directory_name)
    print(f"Directory '{pvd_directory_name}' created successfully.")
except FileExistsError:
    print(f"Directory '{pvd_directory_name}' already exists.")
except PermissionError:
    print(f"Permission denied: Unable to create '{pvd_directory_name}'.")
except Exception as e:
    print(f"An error occurred: {e}")

outputs_str = "disp vel accel"
op.recorder('PVD', pvd_directory_name, '-precision', 6, '-precision', 6, outputs_str)
